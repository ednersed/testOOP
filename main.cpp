    ...
    if (hwnd != NULL)
    {
        ...
        if (hdc != 0)
        {
            ...
            if (rocket1.CheckCollision(&heavy)) {
                heavy.TakeDamage(rocket1.GetPower());
                cout << "HeavyTank current health: " << heavy.GetHealth() << endl;
                cout << "HeavyTank got hit. Current health: " << heavy.GetHealth() - 50 << endl;
            }
            Sleep(1000);
            // heavy.Hide();  // ÍÅ ÑÒÈÐÀÅÌ!
            // light.Hide();  // ÍÅ ÑÒÈÐÀÅÌ!
            ...
            if (rocket2.CheckCollision(&stealthTank)) {
                stealthTank.TakeDamage(rocket2.GetPower());
                cout << "StealthTank ñurrent health: " << stealthTank.GetHealth() << endl;
                cout << "StealthTank got hit. Current health: " << stealthTank.GetHealth() - 50 << endl;
            }
            Sleep(1000);
            // rocketTank.Hide(); // ÍÅ ÑÒÈÐÀÅÌ!
            // stealthTank.Hide();// ÍÅ ÑÒÈÐÀÅÌ!

            // ----------- ÑÒÈÐÀÅÌ ÂÑÅ ÒÀÍÊÈ ÏÅÐÅÄ ÂÛÕÎÄÎÌ -----------
